  ...
  do i=1,nbasis
   do k=i,nbasis
    ...
    if(i_value.ne.0)then
     ...
     factor2 = 1.d0/(g_min(i)+g_min(k))**1.5d0
     do kw=1,nw
      factor= factor2 * rjacob(kw)/pi_0(kw)
 ...
 do i=1,nbasis
  do k=i,nbasis
   ...
   if(finite_range)i_value=int_ij_zero_rc(i,k)
    ...
    S_ij_ZV2(i,k)=S_ij_ZV2(i,k)+e_tot_Sij*e_tot_Sij
    V_ij_ZV(i,k) =V_ij_ZV(i,k) +e_tot_Vij
    V_ij_ZV2(i,k)=V_ij_ZV2(i,k)+e_tot_Vij*e_tot_Vij
    K_ij_ZV(i,k) =K_ij_ZV(i,k) +e_tot_Kij
    K_ij_ZV2(i,k)=K_ij_ZV2(i,k)+e_tot_Kij*e_tot_Kij
  enddo
 enddo
...
factor = 1.d0 /dsqrt(dfloat(nbl))
factor2 = 1.d0 /dfloat(nbl)
do i=1,nbasis
 do k=1,nbasis
  S_ij_ZV (i,k) =S_ij_ZV(i,k)*factor2
  S_ij_ZV2(i,k)= S_ij_ZV2(i,k)*factor2
  V_ij_ZV (i,k) =V_ij_ZV(i,k)*factor2
  V_ij_ZV2(i,k)= V_ij_ZV2(i,k)*factor2
  K_ij_ZV (i,k) =K_ij_ZV(i,k)*factor2
  K_ij_ZV2(i,k)= K_ij_ZV2(i,k)*factor2
  error_Sij(i,k)=dsqrt( dabs(S_ij_ZV2(i,k)-S_ij_ZV(i,k)*S_ij_ZV(i,k))) * factor
  error_Vij(i,k)=dsqrt( dabs(V_ij_ZV2(i,k)-V_ij_ZV(i,k)*V_ij_ZV(i,k))) * factor
  error_Kij(i,k)=dsqrt( dabs(K_ij_ZV2(i,k)-K_ij_ZV(i,k)*K_ij_ZV(i,k))) * factor
 enddo
enddo
...
if(i_el.eq.1) then
  if (basis_type == 'STO' .and. alpha_ZV == 1.d0) then
   do kw=1,nw
    r1_mod=dsqrt(r1(kw,1)*r1(kw,1)+r1(kw,2)*r1(kw,2)+r1(kw,3)*r1(kw,3))
   ...
   do kw=1,nw
    ...
    rjacob(kw)=f1*f1*dabs(f1+r1_mod*f1p)
   enddo
  endif
else
  ...
  if (basis_type == 'STO' .and. alpha_ZV == 1.d0) then
   ...
   do kw=1,nw
    ...
    rjacob(kw)=f1*f1*dabs(f1+r1_mod*f1p)*f2*f2*dabs(f2+r2_mod*f2p)
   enddo
  endif
endif
end

subroutine compute_f_fp_jacob_sto(r,i,k,ic,f,fp)
include 'j.inc'

 factor = 0.5d0/dsqrt(g_min(i)+g_min(k))
 if (alpha_ZV == 1.d0) then
   f=a_ZV(i,k)*r * factor
 else
   f=a_ZV(i,k)*r**alpha_ZV * factor
 endif
 if(ic.eq.0)return
 if (alpha_ZV == 1.d0) then
   fp=a_ZV(i,k) * factor
 else
   fp=alpha_ZV*a_ZV(i,k)*r**(alpha_ZV-1.d0) * factor
 endif
...
include 'j.inc'
 ...
 if(basis_type.eq.'STO')then
   call compute_f_fp_jacob_sto(r,i,k,ic,f,fp)
 else if((basis_type.eq.'GTO').and.finite_range)then
