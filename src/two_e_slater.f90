...
!!!
      ...
      implicit none
      integer n,k
      double precision gam,doublefact,fact
      gam=doublefact(2*(n+k)+1)
...
implicit none
integer n
double precision g,pi,doublefact,expo
pi=dacos(-1.d0)
expo=0.5d0*dfloat(n+1)
crochet=doublefact(n-1)/(2.d0*g)**expo
...
if(r12.lt.0.1)fit_12=1.d0/r12
      ...
      enddo
         ...
         if(n.eq.0)return
         rinteg_1=0.d0
         if(mod(n,2).eq.0)rinteg_1=doublefact(n-1)
         end
...
double precision pi
double precision doublefact,factmichel
pi=dacos(-1.d0)
k_parity=mod(k,2)
l_parity=mod(l,2)
if(k_parity.eq.0.and.l_parity.eq.0)then
 n=k/2
 m=l/2
 sin_k_cos_l=pi/2.d0**(n+m+1)*doublefact(k-1)*doublefact(l-1)/factmichel(n+m)
 return
endif
if(k_parity.eq.0.and.l_parity.eq.1)then
 n=k/2
 m=(l-1)/2
 sin_k_cos_l=doublefact(2*m)*doublefact(k-1)/doublefact(k+l)
 return
endif
if(k_parity.eq.1.and.l_parity.eq.0)then
 n=(k-1)/2
 m=l/2
 sin_k_cos_l=doublefact(2*n)*doublefact(l-1)/doublefact(k+l)
 return
endif
